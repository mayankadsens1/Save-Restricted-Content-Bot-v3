    ...
    if FORCE_SUB:
        try:
          ...
          if str(user.status) == "ChatMemberStatus.BANNED":
              await message.reply_text("You are Banned. Contact -- @oo7jatji ")
              return 1
        except UserNotParticipant:
            link = await app.export_chat_invite_link(FORCE_SUB)
            caption = f"Join our channel to use the bot"
            await message.reply_photo(photo="https://ibb.co/jHLQ5rB",caption=caption, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Join Now...", url=f"{link}")]]))
            return 1
...
async def terms(client, message):
    terms_text = (
        ...
        "✨ Payment to us **__does not guarantee__** authorization for the /batch command. All decisions regarding authorization are made at our discretion and mood and There Is No Any Refund Policy.\n"
    )
...
async def plan(client, message):
    plan_text = (
        "> 💰 **Premium Price**:\n\n Frist Contact Admin @oo7jatji Starting from $3 or 250 INR accepted via **__ UPI Or PayPal__** (terms and conditions apply).\n"
